...
parser = argparse.ArgumentParser(
    prog="ABPden",
    description=("Compute density field for ABP simulations.\n"
                 "\n"
                 "Different kernels and cutoff can be specified and the field " \
                 "is stored in density.npy in the root directory."
    )
)
parser.add_argument(
...
def main():
    ...
    snapshots = shift_in_mainbox(snapshots[:, :, frames], L)
    ...
    for idx in tqdm(np.ndindex(density.shape[:-1]), total=np.prod(density.shape[:-1])):
        ...
        r = minimum_image_diff(snapshots[:, :, :n_iters], coords, L)
